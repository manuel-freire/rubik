...
class Rubik {
	...
	Cubito *cubitos; // coordenada "xyz"
	
	// devuelve el cubito de la coordenada x y z
	inline Cubito &at(int x, int y, int z) {		
		return cubitos[x + y*dim + z*dim*dim];
	}	
	// version const (usada automaticamente desde metodos const)
	const inline Cubito &at(int x, int y, int z) const {
		return cubitos[x + y*dim + z*dim*dim];
	}
...
public:
	...
	Rubik(int dimension) : dim(dimension) {
		cubitos = new Cubito[dim*dim*dim];
    ...
    // constructor de copia (innecesario si usas hash)
	Rubik(const Rubik &otro) {
		dim = otro.dim;
		cubitos = new Cubito[dim*dim*dim];		
		for (int i=0; i<dim*dim*dim; i++) {
			cubitos[i] = otro.cubitos[i];
		}
	}
	
    // destructor (innecesario si usas hash)
	~Rubik() {
		delete[] cubitos;
		cubitos = 0;
	}
	
    // operador de copia (innecesario si usas hash)
	Rubik &operator=(const Rubik &otro) {
		for (int i=0; i<dim*dim*dim; i++) {
			cubitos[i] = otro.cubitos[i];
		}
		return *this;
